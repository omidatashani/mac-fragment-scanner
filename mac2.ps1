...
if ($os -ne "Darwin") {
    Write-Host "Unsupported operating system. Exiting script."
    exit
}

# Path to the xray executable and config file in the same folder as the script
$XRAY_PATH = Join-Path -Path $PSScriptRoot -ChildPath "xray"
...
if (-Not (Test-Path -Path $XRAY_PATH)) {
    Write-Host "Error: xray executable not found at path: $XRAY_PATH"
    exit
}
